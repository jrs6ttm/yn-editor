    ...
    if (state.style != null && typeof(Zlib) !== 'undefined') {
        ...
        if (shape != null && shape.substring(0, 8) == 'stencil(') {
            try {
                ...
                console.log(mxResources.get('errorInShape') + ': ' + e);
            }
        }
    }
...
/**
 * Allows to save the graph
 * @type {boolean}
 */
Graph.prototype.savable = true;

/**
 * merge cells
 *
 * */
Graph.prototype.mergeCells = function(merge,border,cells) {
    cells = this.getSelectionCells();
    if(this.compareCells(cells)){
        var pos = this.minimumCell(cells);
        var labels = cells[pos].getAttribute('label');
        for(var i=0;i<cells.length;i++){
            if(i==pos){
                continue;
            }else{
                labels = labels +'\n'+cells[i].getAttribute('label');
                var model = this.getModel();
                model.beginUpdate();
                cells[pos].setAttribute('label',labels);
                model.endUpdate();
            }
        }
        this.deleteCell(cells,pos);
    }
};
Graph.prototype.compareCells = function(cells){
    for (var i = 0; i < cells.length; i++) {
...
Graph.prototype.minimumCell = function(cells){
    var min = cells[0].id;
    var pos = 0;
...
Graph.prototype.deleteCell = function(cells,pos){
    for(var i=0;i<cells.length;i++){
        if(i == pos){
            continue;
        }else{
            this.removeCells([cells[i]]);
        }
    }
    return cells;
};
 ...
 */
Graph.prototype.unmergeCell = function(cell,append) {
    cell = (cell != null) ? cell : this.getSelectionCells();
    append = (append != null) ? append : true;
    cell = this.model.getTopmostCells(cell);
    var model = this.getModel();
    var s = this.gridSize;
    var seleCell = [];
    model.beginUpdate();
    try {
            var parent = model.getParent(cell[0]);
            var child = this.moveCells([cell[0]], s, s, true, parent)[0];
            seleCell.push(child);
            if (!append) {
                var index = parent.getIndex(cell[0]);
...
/**
 * Set communcation to the graph.
 * @param communication
 */
Graph.prototype.setCommunication = function(communication){
    this.communication = communication;
    this.communication.addListener('onGraphMessage', mxUtils.bind(this, function (message) {
        this.handleRMMessage(message);
    }));
};
/**
 ...
 */
Graph.prototype.updateCellGeometry = function (id, dx, dy, width, height) {
    var cell = this.getModel().getCell(id);
    //console.log(cell);
 ...
 */
Graph.prototype.updateCellLockState = function (id, state, tooltip) {
    // todo disi
    // state = ['lock', 'unlock', null]
    var cell = this.getModel().getCell(id);
    if (cell) {
        if (!cell.ceLockOverLay) {
            cell.ceLockOverLay = new mxCellOverlay(new mxImage('images/lock.png', 24, 24), tooltip, mxConstants.ALIGN_RIGHT, mxConstants.ALIGN_TOP);
        }
        if (state === 'lock') {
            cell.ecIsLocked = true;
            this.addCellOverlay(cell, cell.ceLockOverLay);
            this.setCellStyles(mxConstants.STYLE_FOLDABLE, 0, [cell]);
            this.setCellStyles(mxConstants.STYLE_EDITABLE, 0, [cell]);
            this.setCellStyles(mxConstants.STYLE_BENDABLE, 0, [cell]);
            this.setCellStyles(mxConstants.STYLE_MOVABLE, 0, [cell]);
            this.setCellStyles(mxConstants.STYLE_RESIZABLE, 0, [cell]);
            this.setCellStyles(mxConstants.STYLE_ROTATABLE, 0, [cell]);
            this.setCellStyles(mxConstants.STYLE_CLONEABLE, 0, [cell]);
            this.setCellStyles(mxConstants.STYLE_DELETABLE, 0, [cell]);

        } else {
            cell.ecIsLocked = false;
            this.removeCellOverlay(cell, cell.ceLockOverLay);
            this.setCellStyles(mxConstants.STYLE_MOVABLE, 1, [cell]);
            this.setCellStyles(mxConstants.STYLE_FOLDABLE, 1, [cell]);
            this.setCellStyles(mxConstants.STYLE_EDITABLE, 1, [cell]);
            this.setCellStyles(mxConstants.STYLE_BENDABLE, 1, [cell]);
            this.setCellStyles(mxConstants.STYLE_MOVABLE, 1, [cell]);
            this.setCellStyles(mxConstants.STYLE_RESIZABLE, 1, [cell]);
            this.setCellStyles(mxConstants.STYLE_ROTATABLE, 1, [cell]);
            this.setCellStyles(mxConstants.STYLE_CLONEABLE, 1, [cell]);
            this.setCellStyles(mxConstants.STYLE_DELETABLE, 1, [cell]);
        }
    }

};
/**
 * Hight light the running task
 * @param ids::Array
 * @param type
 * @param tooltip
 */
Graph.prototype.hiLightRunningTask = function (ids, type, tooltip, msgHandler) {
    var me=this;
    if(ids.length>0){
        this.resetRunningTask();
        if(ids[0]!==null){
            for(var i = 0; i < ids.length; i++){
                var cell = this.getModel().getCell(ids[i]);
                if (cell && cell.isEcTaskRunning) {
                    if (!cell.ceRunningOverLay) {
                        cell.ceRunningOverLay = new mxCellOverlay(new mxImage('images/play.png', 24, 24), tooltip, mxConstants.ALIGN_RIGHT, mxConstants.ALIGN_BOTTOM);
                    }
                    this.addCellOverlay(cell, cell.ceRunningOverLay);
                    cell.ceRunningOverLay.addListener(mxEvent.CLICK, function(sender, evt){
                        //communication.iAmEnd
                        var fileId=editor.getFileId();
                        var temp=[];
                        temp.push(fileId+':'+ids[0]);
                        me.communication.endMe(temp);
                    });
                }
            }
        }else{
            var message = new BasicMessage(true, null, mxResources.get('noNextTask'), null);
            msgHandler(message);
            this.resetRunningTask();
        }
        ...
        // fangzhou todo[dev]
        // Develope a dedicate message dialog for showing simple messages like the following.
        var message = new BasicMessage(true, null, mxResources.get('noNextTask'), null);
        msgHandler(message);
        this.resetRunningTask();
    }
...
/**
 * Remove the running icon
 */
Graph.prototype.resetRunningTask = function(isForce){
    var allCells = this.findAllCellsUndParent();
    for(var i = 0; i < allCells.length; i++){
        if(isForce || allCells[i].isEcTaskRunning === false){
            if(allCells[i].ceRunningOverLay){
                this.removeCellOverlay(allCells[i], allCells[i].ceRunningOverLay);
                delete allCells[i].ceRunningOverLay
            }
        }
    }
};
...
Graph.prototype.setIsEcTaskRunning = function(ids, value){
    var cell = this.getModel().getCell(ids[0]);
    cell.isEcTaskRunning = value;
};
/**
 * Check if there are lock cells
 * @returns {boolean}
 */
Graph.prototype.checkIfHasLockedCell = function () {
    var allCells = this.findAllCellsUndParent();
    for(var i = 0; i < allCells.length; i++){
        if(allCells[i].ecIsLocked){
...
/**
 * Retrieve all cells under certain parent domain.
 * Given no parameter indicates find all cells of a graph
 * @param parent
 * @param result
 * @returns []
 */
Graph.prototype.findAllCellsUndParent = function (parent, result) {
    result = (result != null) ? result : [];

    var model = this.getModel();

    if (parent == null) {
        // init root cell as the most outside cell
        parent = this.getCurrentRoot();
    ...
    if (parent != null) {
        ...
        for (var i = 0; i < childCount; i++) {
            // when no chid anymore, don't recursive anymore
            var cell = model.getChildAt(parent, i);

            //console.log(cell);
            result.push(cell);
            this.findAllCellsUndParent(cell, result);
        }
    }
...
var currentIds=[];
Graph.prototype.startProcessSimu = function(){
    currentIds=[];
    var fileId=editor.getFileId();
    this.communication.getStartFigure(fileId);
};

Graph.prototype.goToNextStep = function () {
    if (this.startTaskId) {
        this.setIsEcTaskRunning(this.startTaskId, false);
    }
    var temp=[];
    temp.push(editor.getFileId() + ':' + currentIds[0]);
    this.communication.getNextFigure(temp);
};

Graph.prototype.endAllStep = function(fileId){
    this.communication.endAllStep(fileId);
};
Graph.prototype.handleRMMessage = function (message) {
    var me = this;
    //console.log(message);
    if (message.success === true) {
        if (message.type === 'getStartFigures') {
            console.log(message.data.startFigures);
            //currentIds=message.data.startFigures[0].id;
            me.startTaskId = message.data.startFigures[0].id;
            currentIds.push(message.data.startFigures[0].id);
            me.setIsEcTaskRunning(currentIds, true);
            this.hiLightRunningTask(currentIds);
        } else if (message.type === 'TaskStarted') {
            currentIds = [];
            if (message.data.figure !== null) {
                //console.log(message.data);
                var figure = message.data.currentTask.uiMetaData;
                var id=figure.id.split(':')[1];
                currentIds.push(id);
                console.log(message.data);
                if (message.data.fileId) {
                    //console.log('currentId'+currentIds);

                    //this.communication.retrieveGraphModel({gFileId: message.data.fileId}, mxUtils.bind(editor, editor.setGraphModel), function () {
                    //    me.setIsEcTaskRunning(currentIds, true);
                    //    me.hiLightRunningTask(currentIds);
                    //
                    //});
                    var queryObj = appUtils.convertQueryStrToJSON();
                    queryObj.gFileId = message.data.fileId;
                    History.pushState(queryObj, this.communication.apis.retrieveGraphModel, appUtils.convertJSONToQueryStr(queryObj, '?'));
                ...
                if (message.data.isEnd) {
                    //me.endAllStep();
                    me.hiLightRunningTask(currentIds, null, null, function (msg) {
                        alert(msg.msg);
                        //EditorUi.showDialog(new messageDialogBody(EditorUi, msg.msg),300,null,true,true);
                    });
                } else {
                    console.log('no next');
                }
        ...
        } else if (message.type === 'TaskEnded') {
            currentIds = [];
            //new
            //console.log(message.data.figure);
            var figure = message.data.currentTask.uiMetaData;
            var id=figure.id.split(':')[1];
            currentIds.push(id);
            console.log(figure.id);

            me.setIsEcTaskRunning(currentIds, false);
            me.hiLightRunningTask(currentIds);

        } else if (message.type === 'allIsEnd') {
...
if (typeof mxVertexHandler != 'undefined') {
            ...
            if (tmp != null) {
                ...
                for (var i = 0; i < attrs.length; i++) {
                    if (mxUtils.indexOf(ignored, attrs[i].nodeName) < 0 && attrs[i].nodeValue.length > 0) {
                        ...
                        if(key === 'BasicInfo' && attrs[i].nodeValue ){
                            var basicInfo = JSON.parse(attrs[i].nodeValue)
                            tip += mxResources.get('learningContent') + ': ' + mxUtils.htmlEntities(basicInfo.content) + '\n';
                        }
                    }
                }
    ...
    (function () {
        ...
        if ('contentEditable' in document.documentElement) {
            ...
            EditorUi.prototype.init = function () {
                ...
                mxEvent.addListener(this.editor.graph.container, 'keypress', mxUtils.bind(this, function (evt) {
                    ...
                    if (!this.editor.graph.isEditing() && !this.editor.graph.isSelectionEmpty() && evt.which !== 0 && !mxEvent.isAltDown(evt) && !mxEvent.isControlDown(evt) && !mxEvent.isMetaDown(evt)) {
        ...
        mxVertexHandler.prototype.isRecursiveResize = function (state, me) {
            return !this.graph.isSwimlane(state.cell) && this.graph.model.getChildCount(state.cell) > 0 && !mxEvent.isControlDown(me.getEvent()) && !this.graph.isCellCollapsed(state.cell) &&
        ...
        EditorUi.prototype.init = function () {
            ...
            mxVertexHandler.prototype.init = function () {
                ...
                // !! register two important listerner for graph operations
                this.selectionHandler = mxUtils.bind(this, function (sender, evt) {
                    update();
                });

                // fire update when the celection changed
                this.graph.getSelectionModel().addListener(mxEvent.CHANGE, this.selectionHandler);

                this.modelChangeHandler = mxUtils.bind(this, function (sender, evt) {
                    this.updateLinkHint(this.graph.getLinkForCell(this.state.cell));
                    update();
                });

                // fire this.changeHandler when model changed. the model maps the whole graph
                ...
                if (this.graph.isEnabled() && (touchStyle || urlParams['connect'] == null || urlParams['connect'] == '2')) {
                    ...
                    if (showConnectorImg && this.graph.isCellConnectable(this.state.cell)) {
                        ...
                        if (mxUi === 'subject_design'){
                            this.connectorImg.setAttribute('title', mxResources.get('subjectPlusTooltip'));
        ...
        // override the mx method: mxVertexHandler.prototype.redrawHandles
        var vertexHandlerRedrawHandles = mxVertexHandler.prototype.redrawHandles;
        mxVertexHandler.prototype.redrawHandles = function () {
            vertexHandlerRedrawHandles.apply(this);

            // body of overriding: config func for draging right blue "plus" mark
            if (this.state != null && this.connectorImg != null) {
                var s = this.state;

                //console.log(s.cell);
                if (s.cell.ecIsLocked) {
                    return;
                }
