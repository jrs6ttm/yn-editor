 ...
 * ++: The east panel
 */
PropertyForm = function(editorUi, container) {
 ...
 */
PropertyForm.prototype.labelIndex = 0;

/**
 * Returns information about the current selection.
 */
PropertyForm.prototype.currentIndex = 0;

/**
 * Adds the label menu items to the given menu and parent.
 */
PropertyForm.prototype.init = function() {
	...
	this.update = mxUtils.bind(this, function(sender, evt) {
		...
		this.refresh();
		var currentSelection = this.editorUi.editor.graph.getSelectionCells();
		//appUtils.getMxCellPersistObj(currentSelection);
		if(urlParams['ch']){
			if (currentSelection.length>0) {
				console.log(currentSelection);
				for(var i=0;i<currentSelection.length;i++){
					currSelection(currentSelection[i]);
				}
			} else {
				var temp='false';
				currSelection(temp);
		...
		//	ui.communication.getNextFigure(id,function(res){
		//		console.log(res);
		//	});
		//}else{
		//	//var xml = mxUtils.getXml(editor.getGraphXml());
		//	ui.communication.parseSequence(function(res){
		//		console.log(res);
		//	});
		//}
	});
 ...
 * Returns information about the current selection.
 */
PropertyForm.prototype.clearSelectionState = function() {
 ...
 * Returns information about the current selection.
 */
PropertyForm.prototype.getSelectionState = function() {
 ...
 * Returns information about the current selection.
 */
PropertyForm.prototype.createSelectionState = function() {
 ...
 * Returns information about the current selection.
 */
PropertyForm.prototype.initSelectionState = function() {
 ...
 * Returns information about the current selection.
 */
PropertyForm.prototype.updateSelectionStateForCell = function(result, cell, cells) {
 ...
 * Returns information about the current selection.
 */
PropertyForm.prototype.isFillState = function(state) {
 ...
 * Returns information about the current selection.
 */
PropertyForm.prototype.isGlassState = function(state) {
	var shape = mxUtils.getValue(state.style, mxConstants.STYLE_SHAPE, null);
 ...
 * Returns information about the current selection.
 */
PropertyForm.prototype.isRoundedState = function(state) {
	var shape = mxUtils.getValue(state.style, mxConstants.STYLE_SHAPE, null);
 ...
 * Returns information about the current selection.
 */
PropertyForm.prototype.isImageState = function(state) {
	var shape = mxUtils.getValue(state.style, mxConstants.STYLE_SHAPE, null);
 ...
 * Returns information about the current selection.
 */
PropertyForm.prototype.isShadowState = function(state) {
	var shape = mxUtils.getValue(state.style, mxConstants.STYLE_SHAPE, null);
 ...
 * Adds the label menu items to the given menu and parent.
 */
PropertyForm.prototype.clear = function() {
 ...
 * Adds the label menu items to the given menu and parent.
 */
PropertyForm.prototype.refresh = function() {
	...
	if (graph.isSelectionEmpty()) {
		...
		label0.style.backgroundColor = backgroundColor;
		label2.style.backgroundColor = backgroundColor;
		label3.style.backgroundColor = backgroundColor;
		...
		// semantics
		if (mxUi === 'task_design') {
			mxUtils.write(label0, mxResources.get('properties'));
		} else {
			mxUtils.write(label0, mxResources.get('semantics'));
		}
		div.appendChild(label0);

		var semanticsPanel = div.cloneNode(false);
		semanticsPanel.style.display = 'none';
		this.panels.push(new SemanticFormPanel(this, ui, semanticsPanel));
		this.container.appendChild(semanticsPanel);

		addClickHandler(label0, semanticsPanel, idx++);

		//comment
		mxUtils.write(label3, mxResources.get('comment'));
		div.appendChild(label3);

		var commentPanel = div.cloneNode(false);
		commentPanel.style.display = 'none';
		this.panels.push(new CommentPanel(this, ui, commentPanel));
		this.container.appendChild(commentPanel);

		addClickHandler(label3, commentPanel, idx++);
 ...
 */
BaseFormPanel = function(format, editorUi, container) {
	this.format = format;
	this.editorUi = editorUi;
	this.container = container;
	this.listeners = [];
 ...
 */
BaseFormPanel.prototype.getSelectionState = function() {
	var graph = this.editorUi.editor.graph;
 ...
 */
BaseFormPanel.prototype.installInputHandler = function(input, key, defaultValue, min, max, unit, textEditFallback, isFloat) {
	unit = (unit != null) ? unit : '';
 ...
 */
BaseFormPanel.prototype.createPanel = function() {
	var div = document.createElement('div');
 ...
 * Adds the given option.
 */
BaseFormPanel.prototype.createTitle = function(title) {
	var div = document.createElement('div');
 ...
 * Adds the given option.
 */
BaseFormPanel.prototype.createOption = function(label, isCheckedFn, setCheckedFn, listener) {
	var div = document.createElement('div');
 ...
 *
 */
BaseFormPanel.prototype.createStepper = function(input, update, step, height, disableFocus) {
 ...
 */
BaseFormPanel.prototype.createCellOption = function(label, key, defaultValue, enabledValue, disabledValue, fn, action) {
 ...
 * Adds the given color option.
 */
BaseFormPanel.prototype.createColorOption = function(label, getColorFn, setColorFn, defaultColor, listener, callbackFn, hideCheckbox) {
	var div = document.createElement('div');
	div.style.padding = '6px 0px 1px 0px';
	div.style.whiteSpace = 'nowrap';
	div.style.overflow = 'hidden';
 ...
 * 
 */
BaseFormPanel.prototype.createCellColorOption = function(label, colorKey, defaultColor, callbackFn, setStyleFn) {
	var ui = this.editorUi;
	var editor = ui.editor;
	var graph = editor.graph;
 ...
 * 
 */
BaseFormPanel.prototype.addArrow = function(elt, height) {
 ...
 * 
 */
BaseFormPanel.prototype.addUnitInput = function(container, unit, right, width, update, step, marginTop) {
 ...
 * 
 */
BaseFormPanel.prototype.createRelativeOption = function(label, key) {
	var graph = this.editorUi.editor.graph;
 ...
 * 
 */
BaseFormPanel.prototype.addLabel = function(div, title, right, width) {
 ...
 * 
 */
BaseFormPanel.prototype.addKeyHandler = function(input, listener) {
 ...
 * 
 */
BaseFormPanel.prototype.styleButtons = function(elts) {
 ...
 * Adds the label menu items to the given menu and parent.
 */
BaseFormPanel.prototype.destroy = function() {
...
CommentPanel = function(format, editorUi, container) {
	BaseFormPanel.call(this, format, editorUi, container);
	this.editorUi = editorUi;
	this.cell = editorUi.editor.graph.getSelectionCell();
	this.init();
};
CommentPanel.prototype.init = function() {
	...
	commentBoxInput.setAttribute('style', 'border: 0;min-height: 75px; padding: 9px 11px 0; width: 210px; resize: none; transition: background 85ms ease-in,border-color 85ms ease-in; overflow: hidden; word-wrap: break-word;');
	...
	submitBtn.onclick = mxUtils.bind(this, function () {
		showCommentDiv.insertBefore(this.createCommentShow(this.editorUi,userName,null, mxResources.get('justNow'),commentBoxInput.value,null),showCommentDiv.firstChild);
		var queryObj = {
			content: commentBoxInput.value,
			taskId: this.cell.id,
			fileId: this.editorUi.editor.getFileId()
		};
		this.editorUi.communication.saveTaskComment(queryObj, mxUtils.bind(this, function (commentData) {
			commentBoxInput.value = null;
			showCommentDiv.replaceChild(this.createCommentShow(this.editorUi, commentData.userData.userName,userId, new Date(commentData.time).toLocaleString(), commentData.content,commentData.id, commentData.userData.userId), showCommentDiv.firstChild);
		}));
	});
	...
	showCommentDiv.setAttribute('style', 'margin-bottom: 20px; overflow-y: auto; height: 600px;');
	this.container.appendChild(showCommentDiv);
	var queryObj = {
		taskId: this.cell.id,
		fileId: this.editorUi.editor.getFileId()
	};
	this.editorUi.communication.loadTaskComment(queryObj, mxUtils.bind(this, function (taskCommentDatas) {
		for (var i =0; i<taskCommentDatas.length; i++) {
			showCommentDiv.insertBefore(this.createCommentShow(this.editorUi,taskCommentDatas[i].userData.userName,userId, new Date(taskCommentDatas[i].time).toLocaleString(), taskCommentDatas[i].content,taskCommentDatas[i].id,taskCommentDatas[i].userData.userId),showCommentDiv.firstChild);
		}
	}));
};
CommentPanel.prototype.destroy = function() {
	BaseFormPanel.prototype.destroy.apply(this, arguments);
...
CommentPanel.prototype.createCommentShow = function (editorUi,userName,userId, time, comment,commentId,commentUserId) {
	...
	timeSpan.setAttribute('style', 'font-size: smaller; color: #A5A5A5; margin-left: 10px;');
	//删除评论按钮
	var delCommentBtn = document.createElement('span');
	//delCommentBtn.className = 'geBtn';
	delCommentBtn.innerHTML = 'x';
	delCommentBtn.title = mxResources.get('delete');
	oldCommentUser.appendChild(userSpan);
	oldCommentUser.appendChild(timeSpan);
	oldCommentUser.appendChild(delCommentBtn);
	delCommentBtn.setAttribute('style','float: right; font-weight: bold; cursor: pointer; color: #707070;width: 15px;text-align: center;');
	delCommentBtn.onmouseover = function () {
		delCommentBtn.style.color = '#4d90fe';
	};
	delCommentBtn.onmouseout = function () {
		delCommentBtn.style.color = '#707070';
	};
	oldCommentUser.setAttribute('style', 'padding: 5px; margin: 5px 0 0 0;');
	var oldCommentContent = document.createElement('div');
	oldCommentContent.innerHTML = comment;
	oldCommentContent.setAttribute('style', 'background-color: #f5f5f5; box-shadow: 0 1px 2px rgba(0,0,0,.23); margin: 0 5px 5px 10px; padding: 5px; word-wrap: break-word; white-space: normal;word-break: normal; color: #707070;text-align: justify;line-height: 150%;');
	...
	delCommentBtn.onclick = mxUtils.bind(this, function () {
		if(userId ===  commentUserId){
			...
			editorUi.communication.removeTaskComment(obj, function () {
				...
				console.log(mxResources.get('DeleteSuccess'));
			});
		}else{
...
mxUtils.extend(TextFormatPanel, BaseFormPanel);
		...
		var btns = [
				...
				function() {
					if (currentTable != null) {
						...
						var dlg = new FilenameDialogBody2(ui, value, mxResources.get('apply'), mxUtils.bind(this, function(newValue) {
 ...
 * Adds the label menu items to the given menu and parent.
 */
SemanticFormPanel = function(format, editorUi, container) {
	BaseFormPanel.call(this, format, editorUi, container);
	this.init();
};
mxUtils.extend(SemanticFormPanel, BaseFormPanel);
SemanticFormPanel.prototype.init = function(){
	var ui = this.editorUi;
	var editor = ui.editor;
	var graph = editor.graph;
	var ss = this.format.getSelectionState();
	...
	var showTextFieldCmpCreator = ui.formItems.genShowTextField(mxResources.get('creator'), 'creator', cell);
	var showTextFieldCmpLink = ui.formItems.genShowTextField(mxResources.get('link'), 'link', cell);
	var showTextFieldCmpDescription = ui.formItems.genShowTextField(mxResources.get('description'), 'description', cell);
	var showTextFieldCmpMember = ui.formItems.genShowTextField(mxResources.get('member'), 'member', cell);
	this.container.appendChild(panel);
	panel.appendChild(showTextFieldCmpCreator.rootContainer);
	panel.appendChild(showTextFieldCmpDescription.rootContainer);
	panel.appendChild(showTextFieldCmpLink.rootContainer);
	panel.appendChild(showTextFieldCmpMember.rootContainer);
};
...
mxUtils.extend(StyleFormatPanel, BaseFormPanel);
...
mxUtils.extend(ArrangePanel, BaseFormPanel);

/**
 * belong to ArrangePanel before 12.18.2015
 */
StyleFormatPanel.prototype.initArrangePanel = function() {
	var graph = this.editorUi.editor.graph;
	var ss = this.format.getSelectionState();
 ...
 *
 */
StyleFormatPanel.prototype.addLayerOps = function(div) {
	var ui = this.editorUi;
 ...
 *
 */
StyleFormatPanel.prototype.addGroupOps = function(div) {
	var ui = this.editorUi;
	var graph = ui.editor.graph;
	var cell = graph.getSelectionCell();
	var count = 0;
 ...
 *
 */
StyleFormatPanel.prototype.addAlign = function(div) {
	var graph = this.editorUi.editor.graph;
 ...
 *
 */
StyleFormatPanel.prototype.addFlip = function(div) {
	var ui = this.editorUi;
	var editor = ui.editor;
	var graph = editor.graph;
	div.style.paddingTop = '6px';
 ...
 *
 */
StyleFormatPanel.prototype.addDistribute = function(div) {
	var ui = this.editorUi;
	var editor = ui.editor;
	var graph = editor.graph;
	div.style.paddingTop = '6px';
 ...
 *
 */
StyleFormatPanel.prototype.addAngle = function(div) {
	var ui = this.editorUi;
	var editor = ui.editor;
	var graph = editor.graph;
	var ss = this.format.getSelectionState();
 ...
 *
 */
StyleFormatPanel.prototype.addEdgeGeometry = function(container) {
	var ui = this.editorUi;
	var graph = ui.editor.graph;
	var rect = this.format.getSelectionState();
...
mxUtils.extend(DiagramFormatPanel, BaseFormPanel);
...
DiagramFormatPanel.prototype.addDocumentProperties = function(div) {
	...
	var btn = mxUtils.button(mxResources.get('image'), function(evt) {
		...
		ui.showDialog(dlg, 360, 200, true, true);
		dlg.init();
