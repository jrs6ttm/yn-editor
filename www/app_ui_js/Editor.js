...
// useLocalStorage will be applied in future
...
Editor = function (chromeless) {
    ...
    this.setLinkTaskFileId = function (taskFileId) {
        if(taskFileId){
            this.linkTaskFileId = taskFileId;
        }
    };
    this.getLinkTaskFileId = function () {
        if(this.linkTaskFileId){
            return this.linkTaskFileId;
        }
    };
    ...
    this.graph.getModel().addListener(mxEvent.CHANGE, mxUtils.bind(this, function () {
        var changes = arguments[1].getProperty('edit').changes;
        //if (mxUi === 'process_design' && (!this.fileId)) {
        //    this.removeCellList.push(this.graph.getModel().cells[2].id);
        //    //this.showDialog(new chooseTaskDialogBody(), 300, null, true, true);
        //}
        ...
        var isPassiveChange = false;
        if (this.passiveChangeList) {
            var currentCellId;
            if (changes[0].cell) {
                currentCellId = changes[0].cell.id;
            } else if (changes[0].child) {
                currentCellId = changes[0].child.id;
            }
            for (var i = 0; i <this.passiveChangeList.length; i++) {
                if (currentCellId && this.passiveChangeList[i] === currentCellId) {
                    isPassiveChange = true;
                    for (var j = i; j <this.passiveChangeList.length - i; j++) {
                        this.passiveChangeList[j] = this.passiveChangeList[j+1];
                    }
                    this.passiveChangeList.pop();
                    break;
                }
            }
        }
        if (!isPassiveChange) {
            ...
            if (changes.length === 3) {
                if(changes[0].cell){
                    isAddNew = (changes[0].cell.id === changes[1].cell.id);
                }
                //isAddNew = (changes[0].cell.id === changes[1].cell.id);
            }
            if (isAddNew === false) {
                ...
                if (room) {
                    for (var i = 0; i < changes.length; i++) {
                        if (changes[i].geometry) {
                            appUtils.getMxGeometryChange(changes[i]);
                        } else if (changes[i].value) {
                            if (changes[i].attribute) {
                                appUtils.getMxCellAttributeChange(changes[i]);
                            } else {
                                appUtils.getMxValueChange(changes[i]);
                            }
                        } else if (changes[i].style) {
                            appUtils.getMxStyleChange(changes[i]);
                        } else if (changes[i].child) {
                            appUtils.getMxChildChange(changes[i]);
                        }
                    }
                }
                ...
                var room = urlParams['ch'];
                if(room){
                    //if(changes[2].previous){
                    //    var changeParent = {
                    //        id: changes[2].child.id,
                    //        parent: changes[2].parent
                    //    };
                    //    syncChanges('changeParent', changeParent, function () {
                    //
                    //    });
                    //}else {
                        var newObj=appUtils.getMxCellPersistObj(changes[0].cell);
                        for(var i=0;i<newObj.length;i++){
                            syncChanges('addNew',newObj[i],function(){
            ...
            /*if (changes.length == 1) {
             ...
             //console.log(room);
             if (room) {
             if (changes[0].geometry) {
             appUtils.getMxGeometryChange(changes[0]);
             } else if (changes[0].value) {
             if (changes[0].attribute) {
             appUtils.getMxCellAttributeChange(changes[0]);
             } else {
             appUtils.getMxValueChange(changes[0]);
             }
             } else if (changes[0].style) {
             appUtils.getMxStyleChange(changes[0]);
             } else if (changes[0].child && changes[0].parent) {
             appUtils.getMxChildChange(changes[0]);
             }
             }
             }*/
 ...
 */
Editor.prototype.defaultScrollbars = (mxUi==='task_design')?false:(!mxClient.IS_IOS);

/**
 * Specifies if the page should be visible for new files. Default is true.
 */
Editor.prototype.defaultPageVisible = !(mxUi==='task_design');
...
/**
 * Gets or create file name
 * @returns {null|*|string}
 */
Editor.prototype.getOrCreateFilename = function () {
    return this.filename || mxResources.get((mxUi==='course_design') ? 'course_designX':((mxUi==='process_design')?'process_designX':'drawing'), [counter]);
 ...
 * @param graphModel
 */
Editor.prototype.setGraphModel = function(graphModel){
    if(graphModel) {
        var doc = mxUtils.parseXml(graphModel.xml);
        this.setGraphXml(doc.documentElement);
        this.setModified(false);
        this.setFilename(graphModel.fileName);
        this.setFileId(graphModel.gFileId);
        this.setLinkTaskFileId(graphModel.taskFileId);
        this.setProcessId(graphModel.processId);
        this.setFileType(graphModel.fileType?graphModel.fileType:mxUi);
        console.log('文件类型：'+this.getFileType());
...
Editor.prototype.resetGraph = function () {
    this.graph.gridEnabled = !(mxUi==='task_design' || mxUi === 'subject_design');
    ...
    this.graph.background = (mxUi==='task_design')?'#bce8f1':((mxUi==='subject_design')?'#eef8f6':((mxUi==='course_design') ? '#FFFFCC' :(mxUi==='process_design') ? '#FFF' :null));
...
Editor.prototype.setModified = function (value) {
    this.modified = value;
    if(this.modified){
        this.setStatus(mxResources.get('changesNotSaved'));
    }else{
        this.setStatus(mxResources.get('saved'));
    }
    if(value === 'break'){
        this.modified = false;
        this.setStatus(mxResources.get('changesNotSaved'));
...
Editor.prototype.setFilename = function (value) {
    this.filename = value;
    this.fireEvent(new mxEventObject('fileNameChanged'));
    document.getElementById('docTitle').innerText = value;
 ...
 * @returns {boolean}
 */
Editor.prototype.validateFileName = function (fileName) {

    var pattern=/^[ ]+$/gi;
    if (fileName != null && fileName.length > 0 && !pattern.test(fileName)){
        return true;
    }
...
Editor.prototype.init = function () {
    ...
    // override the original mxPopupMenu.prototype.addItem = function(title, image, funct, parent, iconCls, enabled, active)
    ...
    mxGraphHandler.prototype.isDelayedSelection = function (cell, me) {
        ...
        if ((psel == null || (psel != cell && psel != parent)) && !this.graph.isCellSelected(cell) && model.isVertex(parent) && !this.graph.isValidRoot(parent)) {
            result = true;
        }
...
(function () {
    ...
    mxStencilRegistry.libraries.bpmn = [SHAPES_PATH + "/mxBpmnShape2.js", STENCIL_PATH + "/bpmn.xml"];
    /**
    ...
    //++
    mxStencilRegistry.getStencil2 = function (name) {
        var result = mxStencilRegistry.stencils[name];
        if (null == result) {
            var basename = mxStencilRegistry.getBasenameForStencil(name);
            if (null != basename) {
                result = mxStencilRegistry.libraries[basename];
                if (null != result) {
                    if (null == mxStencilRegistry.packages[basename]) {
                        mxStencilRegistry.packages[basename] = 1;
                        for (basename = 0; basename < result.length; basename++) {
                            var fileName = result[basename];
                            ".xml" == fileName.toLowerCase().substring(fileName.length - 4, fileName.length) ? mxStencilRegistry.loadStencilSet(fileName, null) : ".js" == fileName.toLowerCase().substring(fileName.length -
                                3, fileName.length) && (fileName = mxUtils.load(fileName), null != fileName && eval.call(window, fileName.getText()))
                        }
                    }
